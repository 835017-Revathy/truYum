...
insert into truyum.menu_item(me_name, me_price,me_active,me_date_of_launch,me_category,me_free_delivery) values('Chocolate Brownie',32.00,'Yes','2012-11-02','Desserts','Yes');
-- b.Retriving menu items(Admin view)
select * from truyum.menu_item;
-- 2.    View Menu Item List Customer(TYUC002)
-- a. Retriving menu items(Customer View)
select * from truyum.menu_item where me_active='Yes' and me_date_of_launch<=(select CurDate());
-- 3. Edit Menu items(TYUC003)
-- a.Retriving particular menu item for edit
...
SET 
    ...
    me_price = 150.00,
    me_active = 'No',
    me_date_of_launch = '2008-03-15',
    me_category = 'Starters',
    me_free_delivery = 'No'
WHERE
    me_id = 1;
-- 4.  Add to Cart(TYUC004)
-- a. Inserting user details adn adding cart information in their user id 
insert into truyum.user(us_name) values('Revathy');
insert into truyum.user(us_name) values('Tejaswini');

insert into truyum.cart(ct_us_id,ct_pr_id) values (1,1);
insert into truyum.cart(ct_us_id,ct_pr_id) values (1,1);
insert into truyum.cart(ct_us_id,ct_pr_id) values (1,3);
-- 5.  View Cart(TYUC005)
-- a.Retriving all the carts from the particular user id
SELECT 
    me.me_id,
    me.me_name,
    me.me_price,
